    ...
    async export(data, editor, W, H, transparent, textSettings = null, legendSettings = null) {
        ...
        if (this.layers.pitLane !== false) {
            ...
            if (pit && pit.length >= 2) {
                ...
                svg += `<path d="${pitPath}" fill="none" stroke="${this.roadColor}" stroke-width="${10 * scale}" stroke-linecap="round" stroke-linejoin="round" />`;
            }
        }

        // Garage
        if (data.garage && this.layers.garages !== false) {
            const g = data.garage;
            const s = ts(g.x, g.y);
            const w = g.width * scale;
            const l = g.length * scale;
            const rot = g.rotation || 0;
            svg += `<g transform="translate(${s.x}, ${s.y}) rotate(${rot})">`;
            svg += `<rect x="${-l/2}" y="${-w/2}" width="${l}" height="${w}" fill="#555" />`;
            svg += `</g>`;
        }
            ...
            data.zones.filter(z => z.type === 'straight_mode').forEach(zone => {
                ...
                if (zone.showLabel !== false) {
                    ...
                    if (pathPts.length > 1) {
                        ...
                        tempCtx.font = `bold ${fontSize * scale}px Outfit`;
                        const charWidthsWorld = [];
                        let totalTextWWorld = 0;
                        for (let c = 0; c < text.length; c++) {
                            const cwScreen = tempCtx.measureText(text[c]).width;
                            const cwWorld = cwScreen / scale;
                            charWidthsWorld.push(cwWorld);
                            totalTextWWorld += cwWorld;
                        }
                        const charGapWorld = 1.0 / scale;
        ...
        if (this.layers.chequeredFlag !== false || this.layers.direction !== false) {
            ...
            if (this.layers.chequeredFlag !== false) {
                ...
                const th = 14 * scale;
                if (chequeredURI) {
                    svg += `<image href="${chequeredURI}" xlink:href="${chequeredURI}" x="${-tw / 2}" y="${-th / 2}" width="${tw}" height="${th}" preserveAspectRatio="none" />`;
                } else {
                    // Procedural fallback
                    ...
                    for (let row = 0; row < 2; row++) {
                        for (let col = 0; col < checks; col++) {
                            ...
                            svg += `<rect x="${-tw / 2 + col * cw}" y="${-th / 2 + row * ch}" width="${cw + 0.5}" height="${ch + 0.5}" fill="${fill}" />`;
                        }
                    }
                }
            }
            ...
            if (this.layers.direction !== false) {
                const ax = 17 * scale;
                const ay = 0;
                const svgScale = 0.9 * scale;
                svg += `<g transform="rotate(-90)">`; // Undo the normal rotation from the parent group so we point forward
                svg += `<g transform="translate(${ax}, ${ay}) scale(${svgScale}) rotate(45) translate(-12, -12)">`;
                svg += `<path d="M21.15,2.86a2.89,2.89,0,0,0-3-.71L4,6.88a2.9,2.9,0,0,0-.12,5.47l5.24,2h0a.93.93,0,0,1,.53.52l2,5.25A2.87,2.87,0,0,0,14.36,22h.07a2.88,2.88,0,0,0,2.69-2L21.85,5.83A2.89,2.89,0,0,0,21.15,2.86ZM20,5.2,15.22,19.38a.88.88,0,0,1-.84.62.92.92,0,0,1-.87-.58l-2-5.25a2.91,2.91,0,0,0-1.67-1.68l-5.25-2A.9.9,0,0,1,4,9.62a.88.88,0,0,1,.62-.84L18.8,4.05A.91.91,0,0,1,20,5.2Z" fill="#fff" />`;
                svg += `</g></g>`;
            }
        ...
        if (this.layers.zones !== false) {
            data.zones.forEach(z => {
                ...
                svg += `<polyline points="${sp.x},${sp.y} ${sp.x},${ly} ${lx},${ly}" fill="none" stroke="#888" stroke-width="1.5" />`;
                if (z.type === 'overtake_activation') {
                    svg += `<circle cx="${sp.x}" cy="${sp.y}" r="${5 * scale}" fill="${zt.color}" />`;
                    svg += `<circle cx="${sp.x}" cy="${sp.y}" r="${2 * scale}" fill="#181818" />`;
                } else {
                    svg += `<circle cx="${sp.x}" cy="${sp.y}" r="${5 * scale}" fill="${zt.color}" stroke="#111" stroke-width="1.5" />`;
                }

                const text = (z.label || zt.label || '').toUpperCase();
                const lines = text.split('\n');
                
                // Estimate text bounding box without DOM
                const estCharWidth = 6.5 * scale;
                const tw = Math.max(...lines.map(l => l.length * estCharWidth)) + 16 * scale;
                const th = lines.length * 16 * scale + 6 * scale;

                svg += `<g transform="translate(${lx}, ${ly}) rotate(${z.rotation || 0})">`;
        ...
        if (this.layers.turnNumbers !== false) {
            data.turnMarkers.forEach(tm => {
                ...
                // Visual road edge is 20 * scale (since total width is 40 * scale). Add 8px gap.
                const distPx = 20 * scale + circleRadiusPx + 8 * scale;
                const sCenter = ts(p.x, p.y);
                const sp = { x: sCenter.x + p.nx * distPx * sgn, y: sCenter.y + p.ny * distPx * sgn };

                svg += `<g transform="translate(${sp.x}, ${sp.y}) rotate(${tm.rotation || 0})">`;
                svg += `<circle cx="0" cy="0" r="${15 * scale}" fill="#ffffff" stroke="#000000" stroke-width="2.5" />`;
                svg += `<text x="0" y="${1 * scale}" fill="#000000" font-family="Outfit" font-size="${13 * scale}" font-weight="bold" text-anchor="middle" dominant-baseline="middle">${tm.label}</text>`;
                if (tm.name) {
                    // Glow filter used to ensure text is readable against track background
                    svg += `<text x="0" y="${-16 * scale}" fill="#ffffff" font-family="Outfit" font-size="${9 * scale}" font-weight="normal" text-anchor="middle" dominant-baseline="middle" filter="url(#glow)">${tm.name.toUpperCase()}</text>`;
                }
                svg += `</g>`;
            });
        }
