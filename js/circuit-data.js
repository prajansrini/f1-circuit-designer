...
F1.ZONE_TYPES = [
    { key: 'straight_mode',       label: 'Straight Mode Zone',    color: '#e10600', bg: '#440200', range: true, multi: true },
    { key: 'overtake_detection',  label: 'Overtake Detection',    color: '#8bc34a', bg: '#2d4a10', range: false, multi: false },
    { key: 'overtake_activation', label: 'Overtake Activation',   color: '#00bcd4', bg: '#004d56', range: false, multi: false },
...
F1.PREVIEW_LAYERS = [
    ...
    { key: 'direction', label: 'Start/Finish', default: true },
...
F1.CircuitData = class CircuitData {
    ...
    addControlPoint(x, y) {
        const pt = { id: this._genId(), x, y, widthLeft: 12, widthRight: 12,
    ...
    addTurnMarker(segIndex, t, label) {
        const tm = { id: this._genId(), segIndex, t, label: label || (this.turnMarkers.length + 1).toString(), name: '' };
    ...
    addZone(type, segIndex, t, labelOffsetX, labelOffsetY) {
        ...
        const zone = { id:this._genId(), type, segIndex, t,
    ...
    _deserialize(d) { this.name=d.name;this.controlPoints=d.controlPoints;this.isClosed=d.isClosed;this.pitLane=d.pitLane;this.grandstands=d.grandstands;this.zones=d.zones||[];this.garages=d.garages||[];this.turnMarkers=d.turnMarkers||[];this.sectorLabels=d.sectorLabels||[
        { sector: 1, labelOffsetX: 40, labelOffsetY: -30 },
        { sector: 2, labelOffsetX: 40, labelOffsetY: -30 },
        { sector: 3, labelOffsetX: 40, labelOffsetY: -30 }
    ...
    clear() { this.controlPoints=[];this.isClosed=false;this.pitLane={points:[],width:8};this.grandstands=[];this.zones=[];this.garages=[];this.turnMarkers=[];this.sectorLabels=[
        { sector: 1, labelOffsetX: 40, labelOffsetY: -30 },
        { sector: 2, labelOffsetX: 40, labelOffsetY: -30 },
        { sector: 3, labelOffsetX: 40, labelOffsetY: -30 }
    ]; }
